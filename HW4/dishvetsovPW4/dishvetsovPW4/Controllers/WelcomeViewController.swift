...
final class WelcomeViewController: UIViewController {
    ...
    // Color components.
    private let colorPaletteView: ColorPaletteView = ColorPaletteView()
    ...
    /// Action which handles the event of press of notes button and navigates user to new contorller.
    @objc
    private func notesButtonPressed() {
        ...
        UIView.animate(
            ...
            initialSpringVelocity: 0,
            animations: {
                self.notesButton.backgroundColor = CustomColors.set(to: .majorBackgroundDarkGray)
                self.notesButton.backgroundColor = CustomColors.set(to: .minorBackgroundLightOrange)
            })
